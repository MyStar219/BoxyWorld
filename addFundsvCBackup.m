...
@synthesize addFundsView,progressPaymentBtn,totalDueLbl,amountFld,swtView,scrollAddFunds,viewtotalDue,btnViewCardDetls;
//@synthesize lblCardlist;
...
- (void)viewDidLoad {
    ...
    //addFundsView
    progressPaymentBtn.layer.cornerRadius=3.0f;
   ...
   // cell.cardNoLbl.textColor = [UIColor blackColor];
    ...
    else
    {
        ...
        if(addFundsListArray.count > 5){
            ...
            height = (addFundsListArray.count - 0)*rowheight;
        }
        
        NSLog(@"rowheight===%f==%lu====%f",rowheight,(unsigned long)addFundsListArray.count,height);
        
        self.lblCardlist.frame = CGRectMake(self.lblCardlist.frame.origin.x + 5, self.lblCardlist.frame.origin.y, self.lblCardlist.frame.size.width - 10, self.lblCardlist.frame.size.height);
        
        self.addFundsView.frame = CGRectMake(self.addFundsView.frame.origin.x + 5, self.addFundsView.frame.origin.y, self.addFundsView.frame.size.width - 10, height);
        
        self.viewtotalDue.frame = CGRectMake(self.viewtotalDue.frame.origin.x + 5, self.addFundsView.frame.origin.y+self.addFundsView.frame.size.height+2, self.viewtotalDue.frame.size.width - 10, self.viewtotalDue.frame.size.height);
        
        self.btnViewCardDetls.frame = CGRectMake(self.btnViewCardDetls.frame.origin.x + 5, self.viewtotalDue.frame.origin.y+self.viewtotalDue.frame.size.height+2, self.btnViewCardDetls.frame.size.width - 10, self.btnViewCardDetls.frame.size.height);
        
        self.progressPaymentBtn.frame = CGRectMake(self.progressPaymentBtn.frame.origin.x + 5, self.btnViewCardDetls.frame.origin.y+self.btnViewCardDetls.frame.size.height+2, self.progressPaymentBtn.frame.size.width - 10, self.progressPaymentBtn.frame.size.height);
...
-(void)selectCardBtn:(UIButton *)sender
{
    ...
    for (int i=0; i<addFundsListArray.count; i++) {
        ...
        else if(i == tag)
        {
            ...
            selectedCard =@"selected";//[NSString stringWithFormat:@"%d",i];
            NSMutableDictionary *cards = [[NSMutableDictionary alloc]init];
            [cards setObject:cardType forKey:@"cardType"];
            [cards setObject:photos forKey:@"photos"];
            [cards setObject:cardNo forKey:@"cardNo"];
    ...
    //finalCard = [NSString stringWithFormat:@"%@",selectedCard];
